module.exports = (bot) => {
    bot.use(async (ctx, next) => {
        ...
        if (ctx.isGroup() && !ctx.msg.key.fromMe) {
            ...
            const existingMember = members.find(member => tools.helper.areJidsSameUser(member.id, senderLid));
            ...
            if (existingMember) {
                ...
                const memberData = groupMembers.find(member => tools.helper.areJidsSameUser(member.id, senderLid));
                members.push({
                    id: senderLid,
                    sent: 1,
