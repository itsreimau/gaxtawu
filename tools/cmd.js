...
async function handleError(ctx, error, useAxios = false, silent = false) {
	if (!silent) {
		...
		if (reportOwner && reportOwner.length > 0) {
			for (const ownerId of reportOwner) {
				await ctx.replyWithJid(ownerId + Baileys.S_WHATSAPP_NET, {
					text:
						`ⓘ ${formatter.italic(isGroup ? `Terjadi kesalahan dari grup: @${groupJid}, oleh: @${senderId}` : `Terjadi kesalahan dari: @${senderId}`)}\n` +
		...
		if (useAxios && error.status !== 200)
			return await ctx.reply(
				`ⓘ ${formatter.italic(config.msg.notFound)}`
			);
	}
	await ctx.reply(
		`ⓘ ${formatter.italic(`Terjadi kesalahan: ${error.message}`)}`
	);
}
