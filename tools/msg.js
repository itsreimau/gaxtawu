...
function convertMsToDuration(ms, requestedParts = null) {
	...
	if (requestedParts && Array.isArray(requestedParts)) {
		...
		for (const [unit, data] of Object.entries(units)) {
			if (unit === "milidetik") {
				if (!hasLargerUnits && data.value > 0)
...
function generateCmdExample(used, args) {
	if (!used || !args)
		return `${formatter.inlineCode("used")} atau ${formatter.inlineCode("args")} harus diberikan!`;
...
function generateInstruction(actions, mediaTypes) {
	if (
		...
		return `${formatter.inlineCode("actions")} dan ${formatter.inlineCode("mediaTypes")} harus berupa array!`;
	...
	const translatedMediaTypeList = mediaTypes.map(
	...
	const instructions = actions.map(
		(action) => `${actionTranslations[action]}`
	);
...
function generatesFlagInfo(flags) {
	if (!flags || typeof flags !== "object")
		return `${formatter.inlineCode("flags")} harus berupa objek!`;
	return (
		"Flag:\n" +
		Object.entries(flags)
			.map(
				([flag, description]) =>
					`- ${formatter.inlineCode(flag)}: ${description}`
			)
			.join("\n")
	);
}
...
function generateNotes(notes) {
	if (!notes || !Array.isArray(notes))
		return `${formatter.inlineCode("notes")} harus berupa string!`;
