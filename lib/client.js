...
class Client {
    ...
    _normalizeMessage(message, event) {
        ...
        return {
            ...
            message: {
                ...message,
                body: helper.getBodyFromMsg(message)
            }
        };
    }
    ...
    async _createSendMessage(jid, content, options = {}) {
        ...
        if (content?.contacts) {
            if (Array.isArray(content.contacts)) {
                const parsed = content.contacts.map(helper.parseContact).filter(Boolean);
                content = {
                    contacts: {
                ...
                const parsed = content.contacts.contacts.map(helper.parseContact).filter(Boolean);
                content = {
                    contacts: {
                ...
                const parsed = helper.parseContact(content.contacts);
                if (parsed)
                    content = {
                        contacts: parsed
                    };
