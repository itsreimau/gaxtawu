...
module.exports = (bot) => {
	bot.use(async (ctx, next) => {
		...
		if (isGroup && !ctx.msg.key.fromMe) {
			...
			consolefy.info(
				`Incoming command: ${ctx.used.command}, from: ${senderId}`
			);
		}
		...
		const restrictions = [
			...
			},
			{
				key: "cooldown",
				condition:
					new Cooldown(ctx, config.system.cooldown, "multi")
						.onCooldown &&
					!isOwner &&
					!senderDb?.premium,
			...
			},
			{
				...
				condition: await (async () => {
					...
					if (
						senderDb?.botGroupMembership?.isMember &&
						now - senderDb?.botGroupMembership?.timestamp < duration
					)
						return senderDb.botGroupMembership.isMember;
					const isMember = await ctx
						.group(config.bot.groupJid)
						.isMemberExist(ctx.sender.lid);
		...
		const permissionChecks = [
			...
			},
			{
				key: "coin",
				condition: (() => {
					if (!config.system.useCoin || isOwner || senderDb?.premium)
						return false;
