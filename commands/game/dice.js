    ...
    code: async (ctx) => {
        ...
        if (isNaN(input) || input < 1 || input > 6)
            return await ctx.reply(
                ...
                `${ctx.text.generateCmdExample(ctx.used, "4")}\n` +
        ...
        if (!isUnlimited && senderDb?.coin < 500) return await ctx.reply(ctx.text.info("Koin Anda tidak cukup! Minimal memiliki 500 koin untuk bermain."));
        ...
        try {
            ...
            await ctx.reply(ctx.text.info(`${responseText} Dadu menunjukkan angka ${result}. ${prizeText}`));
        } catch (error) {
