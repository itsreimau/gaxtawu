		...
		try {
			...
			collector.on("collect", async (collCtx) => {
				...
				if (participantAnswer === game.answer) {
					...
					await collCtx.reply({
						text:
							`ⓘ ${formatter.italic(`Benar! +${game.coin} Koin`)}\n` +
							game.description,
						buttons: playAgain,
					});
					...
					await collCtx.reply({
						text:
							`ⓘ ${formatter.italic(`Anda menyerah! Jawabannya adalah ${tools.msg.ucwords(game.answer)}.`)}\n` +
							game.description,
						buttons: playAgain,
					});
				} else if (
			...
			collector.on("end", async () => {
				if (session.has(ctx.id)) {
					...
					await ctx.reply({
						text:
							`ⓘ ${formatter.italic(`Waktu habis! Jawabannya adalah ${tools.msg.ucwords(game.answer)}.`)}\n` +
							game.description,
						buttons: playAgain,
					});
				}
