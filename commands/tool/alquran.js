...
module.exports = {
	...
	code: async (ctx) => {
		...
		if (isNaN(surat) || surat < 1 || surat > 114)
			return await ctx.reply(
				`ⓘ ${formatter.italic("Surah harus berupa nomor antara 1 sampai 114!")}`
			);

		try {
			const apiUrl = tools.api.createUrl(
				"https://raw.githubusercontent.com",
				`/penggguna/QuranJSON/master/surah/${surat}.json`
			);
			...
			if (ayat) {
				if (ayat.includes("-")) {
					...
					if (
						...
						return await ctx.reply(
							`ⓘ ${formatter.italic("Rentang ayat tidak valid!")}`
						);
					const selectedVerses = verses.filter(
						(vers) =>
							vers.number >= startAyat && vers.number <= endAyat
					);
					if (!selectedVerses.length)
						return await ctx.reply(
							`ⓘ ${formatter.italic(`Ayat dalam rentang ${startAyat}-${endAyat} tidak ada!`)}`
						);
					...
					if (isNaN(singleAyat) || singleAyat < 1)
						return await ctx.reply(
							`ⓘ ${formatter.italic("Ayat harus berupa nomor yang valid dan lebih besar dari 0!")}`
						);
					const verse = verses.find(
						(vers) => vers.number === singleAyat
					);
					if (!verse)
						return await ctx.reply(
							`ⓘ ${formatter.italic(`Ayat ${singleAyat} tidak ada!`)}`
						);
