module.exports = {
	...
	code: async (ctx) => {
		...
		if (!claim)
			return await ctx.reply(
				`ⓘ ${formatter.italic("Hadiah tidak valid!")}`
			);
		if (ctx.sender.isOwner())
			return await ctx.reply(
				`ⓘ ${formatter.italic("Anda sudah memiliki koin tak terbatas!")}`
			);
		if (level < claim.level)
			return await ctx.reply(
				`ⓘ ${formatter.italic(`Anda perlu mencapai level ${claim.level} untuk mengklaim hadiah ini. Levelmu saat ini adalah ${level}.`)}`
			);
		...
		if (remainingTime > 0)
			return await ctx.reply(
				`ⓘ ${formatter.italic(`Anda telah mengklaim hadiah ${input}. Tunggu ${tools.msg.convertMsToDuration(remainingTime)} untuk mengklaim lagi.`)}`
			);
		...
		try {
			...
			await ctx.reply(
				`ⓘ ${formatter.italic(`Anda berhasil mengklaim hadiah ${input} sebesar ${claim.reward} koin! Koin Anda saat ini adalah ${rewardCoin}.`)}`
			);
		} catch (error) {
