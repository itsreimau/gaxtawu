module.exports = {
	...
	code: async (ctx) => {
		...
		if (ctx.sender.isOwner() || senderDb?.premium)
			return await ctx.reply(
				`ⓘ ${formatter.italic("Koin tak terbatas tidak dapat ditransfer.")}`
			);
		if (coinAmount <= 0)
			return await ctx.reply(
				`ⓘ ${formatter.italic("Jumlah koin tidak boleh kurang dari atau sama dengan 0!")}`
			);
		if (senderDb?.coin < coinAmount)
			return await ctx.reply(
				`ⓘ ${formatter.italic("Koin Anda tidak mencukupi untuk transfer ini!")}`
			);
		...
		try {
			...
			await ctx.reply(
				`ⓘ ${formatter.italic(`Berhasil mentransfer ${coinAmount} koin ke pengguna itu!`)}`
			);
		} catch (error) {
