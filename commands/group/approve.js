module.exports = {
	...
	code: async (ctx) => {
		if (ctx.args[0]?.toLowerCase() === "all") {
			...
			if (pendings.length === 0)
				return await ctx.reply(
					`ⓘ ${formatter.italic("Tidak ada anggota yang menunggu persetujuan.")}`
				);
			...
			try {
				...
				return await ctx.reply(
					`ⓘ ${formatter.italic(`Berhasil menyetujui semua anggota (${allJids.length}).`)}`
				);
			} catch (error) {
		...
		const pendings = await ctx.group().pendingMembers();
		const isPending = pendings.some((pending) =>
			tools.cmd.areJidsSameUser(pending.jid, target)
		);
		if (!isPending)
			return await ctx.reply(
				`ⓘ ${formatter.italic("Akun tidak ditemukan di daftar anggota yang menunggu persetujuan.")}`
			);
