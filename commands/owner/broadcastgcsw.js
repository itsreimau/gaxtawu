module.exports = {
	...
	code: async (ctx) => {
		...
		const [checkMedia, checkQuotedMedia] = [
			...
			tools.cmd.checkQuotedMedia(ctx.quoted?.messageType, [
				"image",
				"video",
			]),
		];
		...
		if (ctx.args[0]?.toLowerCase() === "blacklist" && ctx.isGroup()) {
			...
			if (groupIndex > -1) {
				...
				botDb.save();
				return await ctx.reply(
					`ⓘ ${formatter.italic("Grup ini telah dihapus dari blacklist broadcast")}`
				);
			} else {
				...
				botDb.save();
				return await ctx.reply(
					`ⓘ ${formatter.italic("Grup ini telah ditambahkan ke blacklist broadcast")}`
				);
			}
		}

		try {
			const groupJids = Object.values(
				await ctx.core.groupFetchAllParticipating()
			)
				.filter(
					(group) =>
						!blacklist.includes(group.id) &&
						!group.announce &&
						!group.isCommunity &&
			...
			const waitMsg = await ctx.reply(
				`ⓘ ${formatter.italic(`Mengirim siaran ke ${groupJids.length} grup, perkiraan waktu: ${tools.msg.convertMsToDuration(groupJids.length * 1.5 * 1000)}`)}`
			);
			...
			await ctx.editMessage(
				ctx.id,
				waitMsg.key,
				`ⓘ ${formatter.italic(`Berhasil mengirim ke ${groupJids.length} grup.`)}`
			);
		} catch (error) {
