	...
	code: async (ctx) => {
			...
			return await ctx.reply(
				`${tools.msg.generateInstruction(["send"], ["text"])}\n` +
					tools.msg.generateCmdExample(
						ctx.used,
						"https://t.me/addstickers/ReiAyanamiEvangelionCute"
					)
			);
		...
		try {
			...
			const chunkSize = Math.ceil(
				result.stickers.length / Math.ceil(result.stickers.length / 30)
			);
			const stickerChunks = [];
			...
			for (
				let packIndex = 0;
				packIndex < stickerChunks.length;
				packIndex++
			) {
				...
				const stickersPack = await Promise.all(
					chunk.map(async (sticker, i) => ({
						sticker: await createSticker(
							sticker.image_url,
							sticker.emoji,
							`${ctx.msg.key.id}_${packIndex}_${i}`
						),
				...
				await ctx.reply({
					stickerPack: {
						...
						cover: await createSticker(
							result.stickers[0].image_url,
							result.stickers[0].emoji,
							ctx.msg.key.id
						),
