...
async function handleWelcome(bot, welcome, type, isSimulate = false) {
	...
	if (!isSimulate && !["group", "public"].includes(botDb?.mode || "public"))
		return;
	...
	const customText = isWelcome
		? groupDb?.text?.welcome
		: groupDb?.text?.goodbye;
	...
	const profilePictureUrl = await bot.core
		.profilePictureUrl(participantJid, "image")
		.catch(
			() =>
				"https://i.pinimg.com/736x/70/dd/61/70dd612c65034b88ebf474a52ccc70c4.jpg"
		);
	const canvasUrl = tools.api.createUrl(
		"siputzx",
		`/api/canvas/${isWelcome ? "welcomev5" : "goodbyev5"}`,
		{
...
async function handleWarning(
	...
	const senderWarning = warnings.find((warning) =>
		tools.cmd.areJidsSameUser(warning.jid, senderLid)
	);
	...
	if (currentWarnings >= maxWarnings) {
		const isBotAdmin = await ctx
			.group(groupJid, !config.system.selfReply)
			.isBotAdmin();
		if (isBotAdmin) {
			await ctx.reply(
				`ⓘ ${formatter.italic(`Anda telah menerima ${maxWarnings} warning dan akan dikeluarkan dari grup!`)}`
			);
			if (!config.system.restrict) await ctx.group().kick(senderJid);
			groupDb.warnings = warnings.filter(
				(warning) => warning.jid !== senderLid
			);
		} else if (!isBotAdmin) {
			await ctx.reply(
				`ⓘ ${formatter.italic(`${config.msg.botAdmin} Tidak dapat mengeluarkan Anda yang telah mencapai ${maxWarnings} warning.`)}`
			);
		}
	}
...
module.exports = (bot) => {
	...
	bot.ev.once(Events.ClientReady, async (b) => {
		consolefy.success(
			`${config.bot.name} by ${config.owner.name}, ready at ${b.user?.id || b.user?.lid}`
		);
		...
		if (botRestart?.jid && botRestart?.timestamp) {
			const timeago = tools.msg.convertMsToDuration(
				Date.now() - botRestart.timestamp
			);
	...
	bot.ev.on(Events.MessagesUpsert, async (ctx) => {
		...
		if (isGroup || isPrivate) {
			...
			if (senderDb) {
				const senderUid = [
					tools.cmd.generateUID(senderId),
					tools.cmd.generateUID(senderId, false),
				];
				if (!senderDb?.uid || senderDb?.uid !== senderUid[0])
					senderDb.uid = senderUid[0];
				if (!senderDb?.username)
					senderDb.username = `@user_${senderUid[1]}`;
				if (
					senderDb?.premium &&
					Date.now() > senderDb?.premiumExpiration
				) {
					delete senderDb.premium;
					delete senderDb.premiumExpiration;
				}
				if (isOwner || senderDb?.premium) senderDb.coin = 0;
				if (!senderDb?.coin || !Number.isFinite(senderDb?.coin))
					senderDb.coin = 100;
				senderDb.save();
			}

			// Pengecekan mode bot (premium, group, private, self)
			if (botDb?.mode === "premium" && !isOwner && !senderDb?.premium)
				return;
			if (
				botDb?.mode === "group" &&
				isPrivate &&
				...
				if (reportOwner && reportOwner.length > 0) {
					for (const ownerId of reportOwner) {
						await ctx.replyWithJid(
							ownerId + Baileys.S_WHATSAPP_NET,
							{
			...
			if (senderAfk.reason || senderAfk.timestamp) {
				...
				if (timeElapsed > 3000) {
					...
					await ctx.reply(
						`ⓘ ${formatter.italic(`Anda telah keluar dari AFK ${senderAfk.reason ? `dengan alasan ${formatter.inlineCode(senderAfk.reason)}` : "tanpa alasan"} selama ${timeago}.`)}`
					);
					delete senderDb.afk;
					senderDb.save();
				}
			...
			if (isGroup) {
				if (!isCmd || isCmd?.didyoumean)
					consolefy.info(
						`Incoming message from group: ${groupId}, by: ${senderId}`
					); // Log pesan masuk
				...
				const afkMentions = ctx.quoted
					? [ctx.quoted.sender]
					: ctx.getMentioned();
				if (afkMentions.length > 0) {
					for (const afkMention of afkMentions) {
						const mentionAfk =
							ctx.getDb("users", afkMention)?.afk || {};
						if (mentionAfk.reason || mentionAfk.timestamp) {
							const timeago = tools.msg.convertMsToDuration(
								Date.now() - mentionAfk.timestamp
							);
							await ctx.reply({
								text: `ⓘ ${formatter.italic(`Jangan ganggu! @${ctx.getId(afkMention)} sedang AFK ${mentionAfk.reason ? `dengan alasan ${formatter.inlineCode(mentionAfk.reason)}` : "tanpa alasan"} selama ${timeago}.`)}`,
								mentions: [afkMention],
							});
						}
					}
				...
				if (!isCmd && !isOwner && !isAdmin) {
					...
					for (const type of [
						...
						if (groupDb?.option?.[`anti${type}`]) {
							...
							if (!!checkMedia) {
								await ctx.reply(
									`ⓘ ${formatter.italic(`Jangan kirim ${type}!`)}`
								);
								await ctx.deleteMessage(ctx.id, msg.key);
								if (groupAutokick) {
					...
					if (groupDb?.option?.antilink) {
						if (msg.body && tools.cmd.isUrl(msg.body)) {
							await ctx.reply(
								`ⓘ ${formatter.italic("Jangan kirim link!")}`
							);
							await ctx.deleteMessage(ctx.id, msg.key);
							if (groupAutokick) {
								await ctx.group().kick(senderJid);
					...
					if (groupDb?.option?.antispam) {
						...
						const senderSpam = spamData.find((spam) =>
							tools.cmd.areJidsSameUser(spam.jid, senderLid)
						) || {
							jid: senderLid,
							count: 0,
							lastMessageTime: 0,
						...
						const newCount =
							timeDiff < 5000 ? senderSpam.count + 1 : 1;
						...
						if (
							...
							spamData.push(senderSpam);
						groupDb.spam = spamData;

						if (newCount > 5) {
							await ctx.reply(
								`ⓘ ${formatter.italic("Jangan spam, ngelag woy!")}`
							);
							await ctx.deleteMessage(ctx.id, msg.key);
							if (groupAutokick) {
								await ctx.group().kick(senderJid);
							...
							groupDb.spam = spamData.filter(
								(spam) => spam.jid !== senderLid
							);
						}
					...
					if (groupDb?.option?.antitagsw) {
						...
						if (!!checkMedia) {
							await ctx.reply(
								`ⓘ ${formatter.italic("Jangan tag grup di SW, gak ada yg peduli!")}`
							);
							await ctx.deleteMessage(ctx.id, msg.key);
							if (groupAutokick) {
								await ctx.group().kick(senderJid);
	...
	bot.ev.on(Events.UserJoin, async (welcome) =>
		handleWelcome(bot, welcome, Events.UserJoin)
	);
	bot.ev.on(Events.UserLeave, async (welcome) =>
		handleWelcome(bot, welcome, Events.UserLeave)
	);
